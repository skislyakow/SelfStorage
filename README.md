...
## QR-пропуск (доступ к боксу)

После оплаты заказа для аренды генерируется QR-код (`apps/rentals/services.py`,
`RentalOrder.qr_code`), который пользователь видит в личном кабинете
(`/orders/my-rent/`). QR кодирует ссылку на страницу доступа к боксу
(`/orders/box-access/<id>/`): её открывает владелец заказа или персонал
(разовая авторизация на инфокиоске) и открывает дверь своей системой.

- Генерация вызывается при переходе заказа в статус `active` (в `_mark_paid`);
  сбой генерации QR не блокирует оплату.
- Файлы QR лежат в `media/qr/qr_<id>.png`.
- В демо (`seed_demo`) QR генерируется для активных аренд.

### Статус бокса «Открыт / Закрыт» (заглушка логики)

В личном кабинете (`/orders/my-rent/`) для активной аренды есть кнопки
«Открыть бокс» / «Закрыть бокс» — это состояние `RentalOrder.access_status`
(`open`/`closed`), переключаемое по `POST /orders/box-open/<id>/` и
`/orders/box-close/<id>/` (только владелец заказа). Открытие доступно только пока
аренда активна (срок не вышел); если заказ просрочен/завершён — открытие
блокируется с сообщением. QR-код показывается в кабинете, когда бокс открыт.
